...
emp::vector<org_t> FilterImpossible(emp::vector<org_t> & pop, emp::vector<int> & axes) { 
    std::set<org_t> possible;
    
    for (int ax : axes) {
        emp::vector<org_t> best = FindHighest(pop, ax);
        possible = emp::set_union(possible, best);
    }


    emp::vector<org_t> result(possible.begin(), possible.end());
    return result;


}

emp::vector<int> PruneAxes(emp::vector<int> & axes, emp::vector<org_t> & pop, double epsilon = 0) {
...
void TraverseDecisionTree(fit_map_t & fit_map, emp::vector<org_t> & pop, emp::vector<int> axes, emp::vector<int> perm_levels) {
    ...
    axes = PruneAxes(axes, pop);
    perm_levels.push_back(axes.size());

    pop = FilterImpossible(pop, axes);
    ...
    for (int ax : axes) {
        ...
        if (winners.size() == 1) { // Not a tie
            ...
            TraverseDecisionTree(fit_map, winners, next_axes, perm_levels);
        }
    }
}
